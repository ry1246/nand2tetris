...
class JackTokenizer():
    def __init__(self, filepath):
        ...
        with open(filepath[:-5] + "T.myImple.xml", 'w') as writef:
            writef.write('<tokens>\n')
            while 1:
                ...
                while 1:
                    ...
                    if end_i > -1:
                        ...
                        if len(self.remained_line) > 0:
                            return self._pop._pop_token_from_remained_line()
                        else:
                            self._readline()
                            return self._pop_token_from_remained_line()
    ...
    def judge_token(self, judge_token):
        if judge_token in STR_TO_TOKEN:
            return STR_TO_TOKEN[judge_token]
        elif INTEGER_PATTERN.match(judge_token):
            try:
                return IntegerConstant(judge_token)
            except Exception as e:
                self.raise_exception(e.message)
        elif IDENTIFIER_PATTERN.match(judge_token):
            return Identifier(judge_token)
        elif STRING_PATTERN.match(judge_token):
            return StringConstant(judge_token[1:-1])
        else:
            return None
