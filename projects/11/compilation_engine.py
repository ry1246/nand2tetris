...
class CompilationEngine():
    ...
    def compile_class(self):
        ...
        self.compile_class_name = self.compile_class_name().token
    ...
    def compile_class_var_dec(self):
        ...
        self.compile_var_name(dexlaration=True, type=type_token.token, kind=kind)
    ...
    def compile_var_dex(self):
        ...
        var_num += 1
        while self.next_is(Tokens.COMMA):
            self.compile_symbole(Tokens.COMMA)
            self.compile_var_name(declaration=True, type=type_token.token, kind=IdentifierKind.VAR)
            var_num += 1
        self.compile_symbol(Tokens.SEMI_COLON)
    ...
    def compile_subroutine_dec(self):
        ...
        self.write_element_start('subroutineDex')
    ...
    def cpmpile_var_name(self, declaration=False, type=None, kind=None, let=False, call=False):
        ...
        self.write_identifier_info('declaration: %s, kind: %s, index: %d' %(
            declaration, self.symbol_table.kind_of(self.tokenizer.see_nexr().token),
            self.symbol_table.index_of(self.tokenizer.see_next().token)))
        return self.compile_identifier()

    def write_identifer_info(self, value):
        self.write_element('IdentifierInfo', value)

    def compile_parameter_list(self):
        self.write_element_start('parameterList')

        if self.tokenizer.see_next() in [Tokens.INT, Tokens.CHAR, Tokens.BOOLEAN] or isinstance(self.tokenizer.see_next(), Identifier):
            type_token = self.compile_type()
    ...
    def compile_subroutine_body(self, subroutine_name, subroutine_dec_token):
        ...
        self.write_element_ed('subroutineBody')
    ...
    def compile_statement(self):
        if self.next_is(Tokens.LET):
            ...
            if self.next_is(Tokens.LEFT_BOX_BRACKET):
                ...
                elif kind == IdeintifierKind.FIELD:
            ...
            self.compile_symbol(Tokens.LEFT_CURLY_BRACLET)
            self.compile_statements()
            self.compile_symbol(tokens.RIGHT_CURLY_BRACKET)
            ...
            self.write_element_end('doStatemet')
    ...
    def compile_subroutine_call(self):
        if self.next_is(Tokens.LEFT_ROUND_BRACKET, idx=1):
            ...
            self.compile_symbol(Tokens.RIGHT_ROUND_BRACKET)
            self.vmw.write_call("%s.%s" % (self.compiled_class_name, subroutine), argnum + 1)
        else:
    ...
    def compile_term(self):
        ...
        if self.next_type_is(TokensType.INT_CONST):
            ...
            if self.next_is(Tokens.LEFT_BOXBRACKET, idx=1):
    ...
    def compile_symbol(self, tokens):
        ...
        if type(tokens) == list:
            if self.tokenizer.current_token in tokens:
                ...
                return self.tokenizer.curren_token
            else:
                self.raise_syntax_error('')
        else:
    ...
    def compile_keyword(self, tokens):
        ...
        if type(tokens) == list:
            ...
            if self.tokenizer.current_token == tokens:
                self.write_element('keyword', self.tokenizer.curren_token.token_escaped)
                return self.tokenizer.current_token
            else:
                self.raise_syntax_error('')
    ...
    def compile_string_constant(self):
        ...
        if isinstance(self.tokenizer.current_token, Stringconstant):
            ...
            self.vmw.write_push(Segment.CONST, lent(string))
